...
var (
	exp1 = "" //泛微OA weaver.common.Ctrl 任意文件上传漏洞
)
...
var (
	...
	exp3     = []string{} //泛微OA Bsh 远程代码执行漏洞
	payload3 = []string{}
)

var (
	exp4 = "" //泛微OA e-cology 数据库配置信息泄漏漏洞
)

var (
	exp5 = "" //泛微OA WorkflowCenterTreeData接口SQL注入(
)
...
func Exp4(url string) string {
	...
	if k.StatusCode == 200 {
		s = fmt.Sprintf("%s存在", url+exp4)
	} else {
		s = fmt.Sprintf("%s不存在", url+exp4)
	}
	return s
}
